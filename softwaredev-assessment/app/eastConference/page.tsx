...
export default function EastConference() {
  const [eastTeams, setEastTeams] = useState<Team[]>([]);
  
  
    const fetchEastConTeams = async () => {
      if (!API_KEY || !API_HOST) {
      ...
      const res = await fetch(`https://${API_HOST}/teams/?conference=east`, {
    ...
    useEffect(() => {
      ...
      fetchEastConTeams()
        .then((data) => {
          setEastTeams(data.response || []); // Assuming the API returns an array of Pokémon in 'response'
        })
        .catch((error) => {
